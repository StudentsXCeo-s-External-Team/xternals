...
const ROLES = [
  {
    ...
    cta: "Learn More",
    responsibilities: ["Strategic planning", "Division leadership", "Managing key relationships"],
  },
  {
    num: "B",
  ...
  },
  {
    ...
    badge: "NOT OPEN",
    color: "red",
    desc: "Embed within a division and build real skills in one area. Great for people who want to go deep.",
    cta: "Explore Roles",
...
export default function OpenRecruitmentPage() {
  ...
  return (
    <main className="min-h-screen font-sans overflow-x-hidden bg-sxc-navy">
      ...
      <section className="relative h-screen w-full bg-sxc-navy flex flex-col justify-end pb-28 px-6 sm:px-16 overflow-hidden">
        ...
        <span ref={ghostRef} className="absolute left-[-2vw] top-1/2 text-[20vw] font-black leading-none select-none pointer-events-none text-white/2.5" style={{ transition: "transform 0.05s linear" }}>
          REC
        </span>

        {/* Blue BOTTOM stripe instead of side */}
        <div className="absolute bottom-0 left-0 w-full h-[3px] bg-sxc-skyblue z-30" />
        ...
        <div className="relative z-10 max-w-[1400px] mx-auto w-full">
          <div className="flex items-center gap-3 mb-8">
            ...
            <span className="text-sxc-skyblue-light text-xs tracking-widest uppercase font-bold">Open Recruitment</span>
          </div>

          <p className="text-sxc-skyblue tracking-[0.15em] uppercase text-xs font-semibold mb-4">03 — Careers</p>
          <h1 className="text-[11vw] sm:text-[9vw] md:text-[7vw] font-black text-white uppercase leading-[0.88] tracking-tight mb-10">
            Open<br />
            <span className="text-transparent" style={{ WebkitTextStroke: "2px var(--color-sxc-skyblue)" }}>Recruitment.</span>
          </h1>

          <div className="flex flex-col sm:flex-row sm:items-end gap-8 sm:gap-20">
            <p className="text-zinc-400 text-lg max-w-md leading-relaxed">
              Ready to step in? We recruit annually for management roles, and on a rolling basis for everything else.
            </p>
            <div className="flex gap-6 text-white/35 text-xs tracking-widest uppercase font-semibold">
              {ROLES.map(r => (
                <a key={r.num} href={`#role-${r.num}`} className="hover:text-sxc-skyblue-light transition-colors">{r.num} — {r.title.split(" ")[0]}</a>
              ))}
            </div>
          </div>
        </div>
      ...
      <section className="relative bg-sxc-navy text-white px-6 sm:px-16 py-24 overflow-hidden">
        ...
        <span className="absolute left-0 bottom-0 text-[14vw] font-black text-white/2.5 leading-none select-none pointer-events-none">RECRUIT</span>
        ...
        <div className="max-w-[1400px] mx-auto relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-0 border border-white/10">
            {ROLES.map(({ num, title, badge, color, desc, cta, responsibilities }) => (
              <div
                id={`role-${num}`}
                key={title}
                className="border-r border-white/10 last:border-r-0 p-10 flex flex-col gap-6 group hover:bg-white/5 transition-colors duration-300"
              >
                  ...
                  <span className={`text-xs px-3 py-1 font-bold tracking-widest uppercase ${
                    color === "green"
                      ...
                      : color === "red"
                        ...
                        : "bg-sxc-skyblue/10 text-sxc-skyblue-light"
                  }`}>{badge}</span>
                </div>

                <div>
                  <h3 className="text-2xl font-black text-white leading-tight mb-3">{title}</h3>
                  <div className="h-0.5 w-8 bg-sxc-skyblue mb-5 group-hover:w-16 transition-all duration-300" />
                  ...
                  <ul className="space-y-2">
                    {responsibilities.map(r => (
                      <li key={r} className="flex items-center gap-3 text-xs text-zinc-500">
                        <span className="w-1 h-1 bg-sxc-skyblue rounded-full shrink-0" />
                        {r}
                      </li>
                    ))}
                  </ul>
                ...
                <a href="#" className="mt-auto text-sm font-bold text-white flex items-center gap-2 group-hover:gap-4 transition-all duration-300 hover:text-sxc-skyblue-light">
                  {cta} →
                </a>
              </div>
            ))}
          ...
          <div className="mt-16 border border-white/10 p-10">
            <p className="text-sxc-skyblue-light text-xs tracking-widest uppercase font-bold mb-8">Recruitment Timeline</p>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
              {[
                ...
                <div key={label} className="flex flex-col gap-2">
                  <p className={`text-xs font-black tracking-widest uppercase ${accent === "blue" ? "text-sxc-skyblue-light" : "text-zinc-500"}`}>{period}</p>
                  <p className="text-white font-bold text-sm">{label}</p>
                  <div className={`h-0.5 w-8 ${accent === "blue" ? "bg-sxc-skyblue" : "bg-white/20"}`} />
                </div>
              ))}
            </div>
          </div>
      ...
      {/* ── CTA ── */}
      <section className="bg-sxc-navy border-t border-white/10 px-6 sm:px-16 py-16">
        <div className="max-w-[1400px] mx-auto flex flex-col sm:flex-row items-center justify-between gap-6">
          <div>
            <p className="text-zinc-500 text-sm mb-1">Applications open periodically. Keep an eye out.</p>
            <p className="text-white font-bold">Get notified when recruitment opens.</p>
          </div>
          <a href="#" className="shrink-0 bg-sxc-blue hover:bg-sxc-navy text-white font-bold px-8 py-4 text-sm uppercase tracking-[0.15em] transition-colors duration-300">
            Get Notified
          </a>
        </div>
      </section>
